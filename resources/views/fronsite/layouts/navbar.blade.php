...
<head>
  ...
  <meta name="description"
    content="TrendStore - Belanja Online Terpercaya. Temukan produk terbaik dengan harga terjangkau.">
...
<body>
  ...
  <nav class="ts-navbar" id="mainNavbar">
    <div class="container ts-navbar-inner d-flex justify-content-between align-items-center">
      ...
      <!-- Search Bar Wrapper (Centers search and pushes actions to the right) -->
      <div class="flex-grow-1 d-none d-lg-flex justify-content-center mx-4">
        <form action="{{ route('search.product') }}" method="GET" class="ts-search-bar w-100" style="max-width: 560px;">
      ...
      <div class="ts-navbar-actions">
        ...
        <!-- User Auth -->
        @auth
          <div class="ts-user-dropdown d-none d-lg-block">
        ...
        @else
          <a href="{{ route('login') }}" class="ts-btn ts-btn-outline-primary ts-btn-sm d-none d-lg-inline-flex">
            Masuk
          </a>
          <a href="{{ route('register') }}" class="ts-btn ts-btn-primary ts-btn-sm d-none d-lg-inline-flex">
            Daftar
          </a>
        @endauth
    ...
    <div class="ts-navbar-mobile-search d-lg-none">
      <form action="{{ route('search.product') }}" method="GET" class="ts-search-bar w-100">
        <span class="material-symbols-outlined ts-search-icon">search</span>
        <input type="text" name="query" class="ts-search-input" placeholder="Cari produk..."
      ...
      <div class="row mb-4">
        <div class="col-lg-5 col-md-12 mb-4 mb-lg-0">
          ...
          <p style="color: #6D7588; font-size: 14px; line-height: 1.6; max-width: 350px;">
            Destinasi belanja online pilihan Anda. Menawarkan pengalaman berbelanja yang aman, nyaman, dan produk
            berkualitas terbaik.
          </p>
        </div>
        ...
        <div class="col-lg-3 col-md-4 col-12">
          ...
          <div class="d-flex gap-2">
            <a href="#" style="color: #6D7588; text-decoration: none;"><span class="material-symbols-outlined"
                style="font-size: 24px;">public</span></a>
            <a href="#" style="color: #6D7588; text-decoration: none;"><span class="material-symbols-outlined"
                style="font-size: 24px;">share</span></a>
            <a href="#" style="color: #6D7588; text-decoration: none;"><span class="material-symbols-outlined"
                style="font-size: 24px;">thumb_up</span></a>
          </div>
        </div>
      </div>
      <div class="d-flex flex-column flex-md-row justify-content-between align-items-center pt-3"
        style="border-top: 1px solid #E5E7E9;">
  ...
  <!-- Mobile Nav Drawer (slide-out sidebar) -->
  <div class="ts-mobile-drawer" id="mobileDrawer">
    <div class="ts-drawer-header">
      <span class="ts-drawer-title">Menu</span>
      <button class="ts-drawer-close" id="drawerClose">
    ...
    <div class="ts-drawer-body">
      <!-- User Info Section -->
      <div class="ts-drawer-user-section">
        @auth
          <div class="d-flex align-items-center gap-3">
            ...
            <div>
              ...
              <div class="text-muted" style="font-size: 12px; word-break: break-all;">{{ Auth::user()->email }}</div>
            </div>
          </div>
        @else
          <div class="d-flex flex-column gap-2 w-100">
            <a href="{{ route('login') }}" class="ts-btn ts-btn-outline-primary w-100">Masuk</a>
            <a href="{{ route('register') }}" class="ts-btn ts-btn-primary w-100">Daftar</a>
          </div>
        @endauth
      </div>

      <div class="ts-drawer-divider"></div>

      <!-- Navigation Links -->
      <div class="ts-drawer-links">
        <a href="{{ url('/') }}" class="ts-drawer-link">
          <span class="material-symbols-outlined">home</span>
          Beranda
        </a>
        <a href="{{ route('cart.show') }}" class="ts-drawer-link">
          <span class="material-symbols-outlined">shopping_cart</span>
          Keranjang Belanja
          @if(session('cart') && session('cart') > 0)
            <span class="badge bg-danger ms-auto rounded-pill">{{ session('cart') }}</span>
          @endif
        </a>

        @auth
          <a href="{{ route('orders.index') }}" class="ts-drawer-link">
            <span class="material-symbols-outlined">package_2</span>
            Pesanan Saya
          </a>
          <a href="{{ route('profile.index') }}" class="ts-drawer-link">
            <span class="material-symbols-outlined">manage_accounts</span>
            Profil Saya
          </a>
          @if(Auth::user()->id_role == 1 || Auth::user()->id_role == 2)
            <a href="{{ route('get.product.backsite') }}" class="ts-drawer-link ts-drawer-link-admin">
              <span class="material-symbols-outlined text-success">admin_panel_settings</span>
              Admin Panel
            </a>
          @endif
          <div class="ts-drawer-divider"></div>
          <a href="{{ route('logout') }}" class="ts-drawer-link text-danger">
            <span class="material-symbols-outlined">logout</span>
            Keluar
          </a>
        @endauth
  ...
  <!-- Backdrop for drawer -->
  <div class="ts-drawer-backdrop" id="drawerBackdrop"></div>
  ...
  <script>
    ...
    if (mobileToggle && mobileDrawer && drawerClose && drawerBackdrop) {
