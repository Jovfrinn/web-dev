...
<div class="admin-page-header">
  ...
  <p class="admin-page-subtitle">Perbarui informasi produk secara detail.</p>
</div>
  ...
  <div class="row g-4">
      ...
      <div class="admin-card border-0 shadow-sm" style="border-radius: 16px;">
        ...
        <div class="admin-card-body p-4">
          ...
          <div class="mb-4">
            ...
            <input type="text" id="name_product" name="name_product" value="{{ $product->name_product }}" class="form-control form-control-lg bg-light border-0" style="font-size: 15px;" required placeholder="Contoh: Sepatu Sneakers Pria Premium">
          </div>

          <div class="mb-4">
            <label for="description_product" class="form-label fw-semibold text-dark">Deskripsi Lengkap <span class="text-danger">*</span></label>
            <textarea id="description_product" name="description_product" class="form-control bg-light border-0" style="font-size: 15px; min-height: 180px;" required placeholder="Jelaskan spesifikasi, bahan, dan keunggulan produk ini...">{{ $product->description_product }}</textarea>
          </div>
          ...
          <div class="mb-2">
            ...
            <select id="category_id" name="category_id" class="form-select form-select-lg bg-light border-0" style="font-size: 15px;" required>
              ...
              @foreach (getCategory() as $category)
                <option value="{{ $category->id }}" {{ $product->category_id == $category->id ? 'selected' : '' }}>{{ $category->name_categories }}</option>
              @endforeach
            </select>
          </div>
    ...
    <div class="col-lg-4">
      ...
      <div class="admin-card border-0 shadow-sm mb-4" style="border-radius: 16px;">
        ...
        <div class="admin-card-body p-4">
          <label class="form-label fw-semibold text-dark">Upload Gambar</label>
          <div class="position-relative">
            <input type="file" id="image" name="image[]" class="form-control" multiple style="position: absolute; width: 100%; height: 100%; opacity: 0; cursor: pointer; z-index: 2;">
            <div class="d-flex flex-column align-items-center justify-content-center bg-light rounded-3 p-4 text-center border" style="border-style: dashed !important; border-width: 2px !important; border-color: #cbd5e1 !important;">
              ...
              <span class="fw-medium text-dark">Ubah Gambar (Opsional)</span>
              <small class="text-muted mt-1">Format: JPG, PNG. Kosongkan jika tak diubah.</small>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="admin-card-body p-4">
          ...
          <div class="mb-4">
            ...
            <div class="input-group input-group-lg">
              ...
              <input type="text" id="price_display" value="{{ number_format($product->price, 0, '', '.') }}" class="form-control bg-light border-0" style="font-size: 15px;" required placeholder="0">
              <input type="hidden" id="price" name="price" value="{{ $product->price }}">
            </div>
          </div>

          <div class="mb-4">
            <label for="stock" class="form-label fw-semibold text-dark">Stok Tersedia <span class="text-danger">*</span></label>
            <input type="number" id="stock" name="stock" value="{{ $product->stock }}" class="form-control form-control-lg bg-light border-0" style="font-size: 15px;" required placeholder="0">
          </div>
